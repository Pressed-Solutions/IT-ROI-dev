...
foreach ($console_categories as $cat) :
    ...
    if ($custom_field == '1') {
        $term_relationships = $wpdb->get_results("SELECT * FROM `wp_term_relationships` AS t INNER JOIN wp_postmeta AS p ON t.`object_id` = p.post_id WHERE p.meta_key = 'artical_type' AND p.meta_value = 'yes' AND t.term_taxonomy_id =" . filter_value($cat->term_taxonomy_id) . " ORDER BY p.post_id DESC limit 1");
        $post_7 = $wpdb->get_results("SELECT * FROM `wp_posts` WHERE id =" . filter_value($term_relationships[0]->post_id) . " limit 1");
        $postmeta_7 = $wpdb->get_results("SELECT * FROM `wp_postmeta` WHERE post_id=" . filter_value($term_relationships[0]->post_id) . " order by meta_key desc");
        $permalink = "";
        ?>
                        <div>
